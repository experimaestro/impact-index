...
use rstest::rstest;
use xpmir_rust::{index::sparse::{TermImpact, builder::{Indexer, SparseBuilderIndexTrait, load_forward_index}, wand::search_wand}, search::{TopScoredDocuments, ScoredDocument}, base::{TermIndex, ImpactValue}};

use helpers::documents::{create_document, TestDocument, document_vectors};
...
fn test_heap() {
    ...
    assert!(max == ImpactValue::NEG_INFINITY, "Expected -inf got {}", max);
...
fn test_search(#[case] in_memory: bool,
    ...
    let query: HashMap::<usize, ImpactValue> = data.documents[10].terms.iter().map(|x| (x.term_ix, x.weight as ImpactValue)).collect();
